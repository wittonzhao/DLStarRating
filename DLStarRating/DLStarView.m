...
- (id)initWithDefault:(UIImage*)star highlighted:(UIImage*)highlightedStar position:(int)index allowFractions:(BOOL)fractions fractionNumber: (int)number {
  ...
  if (self) {
    [self setTag:index];
    if (fractions) {
      highlightedStar = [self croppedImage:highlightedStar andFractionNumber:number];
      star = [self croppedImage:star andFractionNumber:number];
    }
    self.frame = CGRectMake((star.size.width*index + kEdgeSpace), 0, star.size.width, star.size.height+kEdgeInsetBottom);
...
- (UIImage *)croppedImage:(UIImage*)image andFractionNumber: (int)number {
  float partWidth = image.size.width/number * image.scale;
  int part = (self.tag+number)%number;
...
- (void)centerIn:(CGRect)_frame with:(int)numberOfStars {
  ...
  self.frame = CGRectMake(((size.width + kEdgeSpace)*self.tag) , newY, size.width, size.height);
}
